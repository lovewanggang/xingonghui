	...
	<view class="background">
		...
		<view class="content">
			<view class="group" style="margin-bottom: 0px;padding-left: 15px;border-color: #c7cdd5;">
				<input v-model="name" type="text" style="color:rgba(36,53,87,1)!important;" class="text_1 text_2p" maxlength="11"  placeholder="搜索機構..." />
				<view class="block">
			...
			<view class="content_main">
				<view class="list" v-for="(item, index) in userList" :key='index'>
					<view class="list_left">
						<image v-if="item.company && item.company.logo == null" class="list_img" src="/static/images/accountSet.png" ></image>
						<image v-else-if="item.company" class="list_img" :src="$pubFuc.advertising(item.company.logo)" ></image>
						<image v-else-if="!item.company && item.logo == null" class="list_img" src="/static/images/accountSet.png" ></image>
						<image v-else class="list_img" :src="$pubFuc.advertising(item.logo)" ></image>
						<view class="list_text" v-if="item.company">
							{{item.company.name}}
						</view>
		...
		methods: {
			getList() {//初始化列表
				...
				that.$minApi.Blacklist({
					s:{type: 2,member_id: that.member_id},
					join: 'company'
				}).then((res)=>{
				...
				// 		that.$minApi.Blacklist({
				// 			s:{type: that.type,company_id: that.company_id},
				// 			join: 'member'
				// 		}).then((res)=>{
				// 			that.userList = res.data
				// 			that.userList.map(i => {
			...
			addShield(row){ //添加用戶
				var that = this;
				that.$minApi.Blacklist({
					s:{member_id: that.member_id,
					company_id: row.id,
					type: 2},
					join: 'company'
				}).then((res)=>{
					if(res.data.length>0){
						return this.$pubFuc.showToast('該機構已被屏蔽');
					}else{
