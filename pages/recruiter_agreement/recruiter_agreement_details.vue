		...
		methods: {
			getList() { // 初始化
				if (this.type == 1) {
					...
					this.top_center = '發布協議';
				}
				
				this.$minApi.config().then((res)=>{
					console.log(res)
					if (this.type == 1){
						this.protocol = res.data[0].recruiter_protocol
					} else if(this.type == 2){
						this.protocol = res.data[0].recruiter_privacy
					} else if(this.type == 3){
						this.protocol = res.data[0].recruiter_publish_protocol
					}
				})
			},
			forgect(){ //返回
